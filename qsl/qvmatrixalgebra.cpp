...
double solveConjugateGradient(const QVSparseBlockMatrix &A, QVVector &x, const QVVector &b, const int maxIters, const int minIters, const double minAbsoluteError)
    {
    double residualsNorm = 0.0;
    QVVector r = b - A.dotProduct(x), p = r;

    double temp2 = r.dotProduct(r);
    for (int i = 0; i < maxIters; i++)
        {
        const QVVector temp1 = A.dotProduct(p);
        const double alpha = temp2 / (p.dotProduct(temp1));
        x += p * alpha;
        r += temp1 * (-alpha);

        const double temp3 = r.dotProduct(r);
    residualsNorm = sqrt(temp3);

    // -----------------------------------------------------------
    // std::cout << "Conjugate gradient! " << (A.dotProduct(x) - b).norm2() << "\t" << sqrt(temp3) << std::endl;
    // -----------------------------------------------------------

    if ( (i>=minIters) and (residualsNorm<minAbsoluteError) )
        break;

        p = r + p * temp3 / temp2;
        temp2 = temp3;
    //std::cout << "------------ EOEOEOEOOE" << std::endl;
        }
    return residualsNorm;
    }
#endif
...
double sparseSolve(const QVSparseBlockMatrix &qvspmatrixPre, QVVector &x, const QVVector &bPre,
	...
	if (method == QV_BJPCG)
		...
		blockJacobiPreconditioning(qvspmatrix, b);
		}
