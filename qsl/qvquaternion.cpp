...
QVQuaternion::QVQuaternion(const QVMatrix &R): QVVector(4)
	...
	const double r = sqrt(1 + R(u,u) - R(v,v) - R(w,w));
...
QVQuaternion QVQuaternion::quaternionProduct(const QVQuaternion &other) const
	...
	return QVQuaternion(	w * other_x + x * other_w + y * other_z - z * other_y,
...
QVQuaternion QVQuaternion::conjugate() const
	{
	return QVQuaternion(-operator[](0), -operator[](1), -operator[](2), operator[](3));
	}

QVQuaternion QVQuaternion::inverse() const
	{
	const QVVector q = conjugate();
	return q / q.norm2();
	}
...
QVVector QVQuaternion::rotate(const QVVector &v) const
	{
	const double	norm = norm2(),
			x = operator[](0) / norm,
			y = operator[](1) / norm,
	...
	const QVQuaternion q(x, y, z, w), p(v[0], v[1], v[2], 0), product = q * p * q.conjugate();
